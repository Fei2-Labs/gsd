...
npx @fei2-labs/get-shit-done --cline --global

# Local install (this project only)
npx @fei2-labs/get-shit-done --cline --local
```
...
```bash
npx @fei2-labs/get-shit-done --codebuddy --global
```
...
```bash
npx @fei2-labs/get-shit-done --qwen --global
```
...
```bash
WINDSURF_CONFIG_DIR=~/.codeium/windsurf-next npx @fei2-labs/get-shit-done@latest --windsurf --global
```
...
If `npx @fei2-labs/get-shit-done` fails due to npm outages or network restrictions, see [docs/manual-update.md](manual-update.md) for a step-by-step manual update procedure that works without npm access.
...
**Opt-out / removal:** delete the SessionStart hook entry that references `gsd-update-banner.js` from your runtime's `settings.json` (Claude Code: `~/.claude/settings.json`; Gemini: `~/.gemini/settings.json`). `npx @fei2-labs/get-shit-done --uninstall` removes both the script and the registration in one pass.
