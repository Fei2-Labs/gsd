...
The banner is silent when up-to-date and rate-limits "check failed" diagnostics to once per 24 hours. Removed cleanly by `npx @fei2-labs/get-shit-done --uninstall` or by deleting the SessionStart entry that references `gsd-update-banner.js`.
...
**Part of:** `npx @fei2-labs/get-shit-done`
...
- Cleanly removed by `npx @fei2-labs/get-shit-done --uninstall`.
